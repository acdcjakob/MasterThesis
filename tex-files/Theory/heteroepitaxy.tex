...
\comment{
    Der folgende Absatz ist probably zu lang, dafür dass ich gar kein pseudomorphic growth beobachte, oder?
    ...
    Vielleicht wäre eine Lösung, (\ref{euq:e3-a}), (\ref{euq:e3-m}), (\ref{euq:e3-c}) und (\ref{equ:e3-r}) in eine Art appendix zu tun?
    }
...
\begin{enumerate}
    \item The magnitude of \gls{ip} strain of the thin film material has to be exactly such that the thin film \gls{ip} lattice constants match the substrate \gls{ip} lattice constants.
    \item On the other hand, due to vertical growth, the \gls{oop} stress of the thin film is demanded to be zero.
\end{enumerate}
The resulting \gls{oop} strain as well as non-diagonal elements of the elasticity tensor can be derived by solving the system of equations (\ref{equ:stress-strain}) with these two boundary conditions.
In \citeauthor{grundmann2018} (\citeyear{grundmann2018}), formulas are derived for the unknown strains in the special case of pseudomorphic heterostructures with threefold rhombohedral symmetry
    \cite{grundmann2018}.
For numerical predictions of those strains, the elasticity tensor $C_{ijkl}$ of the thin film compound has to be known.
But depending on the symmetry of the crystal structure, its components collapse into a lower number of independent entries\footnote{
...
Because of its direct influence on the \gls{oop} lattice plane distance and thus on the \gls{XRD} patterns (cf.~\tbd), the strain component perpendicular to the sample surface, $\epsilon_{zz}$, is of particular interest. %!tbd
...
Note that here, $\mathbf{r}=(x,y,z)$ describes coordinates in the laboratory system -- in contrary to \textcite{grundmann2018}, where $\mathbf{r}$ and $\mathbf{r}'$ are used to describe cartesian coordinates in the crystal and laboratory system, respectively.
...
Note that $\epsilon_{xx,a}=\epsilon_{xx,m}$, $\epsilon_{yy,a}=\epsilon_{yy,m}$, and $\epsilon_{yy,c}=\epsilon_{yy,a}$.
For (01.2)-plane (\textit{r}-orientation), the formula gets more complicated and reads \cite{grundmann2020}:
\begin{align}%!tbd
    \label{equ:e3-r}
...
The numerical values, calculated from the lattice constants (Tab.~\tbd) and the elasticity tensor (Tab.~\ref{tab:Cr2O3-elasticityConstants}), are listed in Tab.~\ref{tab:d_strained}
\begin{table}
    \centering
    \caption{Comparison of \gls{oop} lattice plane distances for bulk \ce{Cr2O3} and pseudomorphic \ce{Cr2O3} on \ce{Al2O3}.}
    \begin{tabular}{cccc}
        \toprule
        orientation&$d$&$d_\mathrm{strained}$&$\epsilon_{zz}$\\\midrule
        \textit{c} (00.1)&\tbd&&\\
        \textit{a} (11.0)&&&\\
        \textit{m} (10.0)&&&\\
        \textit{r} (01.2)&&&\\
...
\begin{wrapfigure}{R}{0.35\textwidth}
    \centering
    \includegraphics[width=\linewidth]{grundmann2016_tiltDislocation.png}
    \caption{Edge dislocation with \gls{bv} perpendicular to sample surface \tbd, taken from grundmann2016 \tbd}
...
It's also clear that the dislocation spacing $D$ decreases the tilt \tbd.
...
Note that the $x$-axis points along the $c$-axis for \textit{m}- and \textit{a}-oriented heterostructures, and similarly along the projection of the $c$-axis on the sample surface for \textit{r}-oriented heterostructures.
...
The two relevant slip systems are $\{00.1\}/\frac{1}{3}\langle11.0\rangle$ and $\{11.0\}/\frac{1}{3}\langle1\bar{1}.1\rangle$, which contain the \enquote{basal} and \enquote{prismatic} glide plane, respectively
    \cite{grundmann2020b}.
The former allows for relaxation along the direction containing the projection of the \textit{c}-axis ($x$-axis), whereas the latter allows for relaxation perpendicular to it ($y$-axis).
With $\zeta_F=c_F/a_F$, one can determine two possible independent \glspl{bv} $\mathbf{b}_c$ with differing screw components but otherwise same tilt and edge components $b_{c,\perp}$ and $b_{c,\parallel}$, respectively.
...
Furhtermore, in this case, the \glspl{bv} of the two remaining \textit{r}-planes have opposite tilt components, i.e. they point outwards and inwards of the surface, respectively.
Regarding (\ref{equ:Theory_tiltDislocation}), this will result in no net tilt of the thin film.

\begin{table}
    \centering
    \caption{lorem ipsum lorem ipsum lorem ipsumlorem ipsum lorem ipsum \tbd}
    \begin{tabular}{cll}
        \toprule
