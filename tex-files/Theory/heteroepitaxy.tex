...
If the \gls{ip}\ lattice constants of two isomorphic compounds match at the interface of a heterostructure, one refers to \emph{\gls{pseudomorphic}} growth.
...
Because of its direct influence on the \gls{oop}\ lattice plane distance and thus on the \gls{XRD} patterns of \thetaomega-scans (cf.\ section \ref{Sec:Methods_XRD}), the strain component perpendicular to the sample surface, $\epsilon_{zz}$, is of particular interest.
...
\end{equation}
Assuming pseudomorphic growth of \ce{Cr2O3} on \ce{Al2O3}, one can compare the strained lattice plance distances to the unstrained bulk values, by utilizing \eqref{equ:d_strained}.
...
When the lattice mismatch is not resolved by adaption to the substrate (cf.\ section \ref{sec:Theory_PseudomorphicGrowth}), the strain can be relieved by formation of \emph{dislocations}, which faciliate a relaxed growth mode of the film
    \cite{kneiss2021}.
...
For a \emph{perfect} dislocation\footnote{
    Also referred to as \emph{full} dislocation \cite{grundmann2016}.
}, the \gls{bv} is a lattice translation vector.
Note that in general, dislocations exhibit both edge- and screw-character
    \cite{hull2011}.

Dislocations are not static, but can move (\emph{glide}) inside the crystal.
The movement happens typically inside a plane which has highest density of atoms (\emph{glide plane}) and along the \gls{bv} which is responsible for the dislocation
    \cite{hull2011}.
The arrangement of glide plane and direction of movement is called \emph{slip system}, e.g.\ for hexagonal structures, one finds $\{00.1\}/\langle11.0\rangle$ to be one prevailing slip system
    \cite{hull2011}.
...
The two relevant slip systems are $\{00.1\}/\frac{1}{3}\langle11.0\rangle$ and $\{11.0\}/\frac{1}{3}\langle1\bar{1}.1\rangle$, which contain the \emph{basal} and \emph{prismatic} glide plane, respectively
    \cite{grundmann2020b}.
...
So the prevailing slip system must have (01.2)-plane (\textit{r}-orientation) or (11.2)-plane (\textit{s}-orientation) character, which are called \emph{pyramidal} slip systems.
