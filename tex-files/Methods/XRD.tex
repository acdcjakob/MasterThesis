...
As described in section \ref{Sec:Theory_X-ray_diffraction}, constructive interference of incoming and scattered X-rays may give insight into the symmetry of exposed crystal structures.
This can be utilized for thin film investigation and is called \gls{XRD}.
The \gls{XRD} device used for this work, namely an \textit{X'Pert Pro} ({Malvern Panalytical Ltd.}), as well as the applied scanning techniques will be presented in the following.

The sample of interest is glued to a metallic holder which itself is placed into the shielded \gls{XRD} device and exposed with X-rays at an angle $\omega$ between sample surface and incident radiation.
The surface normal of the sample holder is parallel to the $z$-axis of the laboratory system. 
...
Note that $\theta$ in the \textsc{Bragg} Equation (\ref{Equ:Theory_BraggCondition}) is the same angle as half of $2\theta$.
The scattering plane is depicted in Fig.\,\ref{Fig:Methods_XRD_geometry}a.
The sample can be rotated by $\phi$ (\emph{azimuth}) and $\psi$ around an axis parallel to the surface normal and parallel to the intersection of sample surface and scattering plane, respectively.

% ---
\subsection{\texorpdfstring{\thetaomega}{2theta-omega}-scans}
...
When analyzing 2\texttheta-\textomega\ diffraction patterns, one usually compares the observed reflections to predicted peak positions of the expected phase of the compound which is investigated.
...
Peak shifts may be due to residual stress, substrate induced strain or compositional gradients in the thin film 
    \cite{harrington2021}.
A certain amount of peak broadening is always present due to convergence of the incident beam as well as convolution of K\textalpha\textsubscript{1}- and K\textalpha\textsubscript{2}-radiation (cf.\ section \ref{Sec:Theory_XRays}), which is called instrumental broadening
    \cite{harrington2021}.
...
\begin{enumerate}[label=(\roman*)]
    ...
    This is done before measuring a 2\texttheta-\textomega-scan to achieve more precise results.
    \item When probing for lattice planes which are not parallel to the sample surface, called \emph{asymmetric reflections}, one can apply the expected inclination angle as an offset $\omega_0$.
    This is the case in Fig.\,\ref{Fig:Methods_XRD_geometry}b.
    \item When $2\theta$ is fixed, but $\omega_0$ is varied, an \emph{\textomega-scan} is performed, which enables quantification of mosaicity (cf.\ section \ref{Sec:Methods_omegaScan}).
\end{enumerate}

% ---
\subsection{\texorpdfstring{\textomega}{omega}-scans}
...
In particular, recording Rocking curves of symmetric and asymmetric lattice planes allows the calculation of dislocation densities in the thin film
    \cite{srikant1997}.
% ---
\subsection{\texorpdfstring{\textphi}{phi}-scans}
...
% ---
Because the \gls{bc} is equal to the description of diffraction with $\mathbf{k}'-\mathbf{k}$ and reciprocal space vectors $\mathbf{K}_{hkl}$ (cf.\ section \ref{Sec:Theory_X-ray_diffraction}), both can be used depending on context.
...
Consequently, this is called a \acrlong{RSM}.
...
With \eqref{Equ:Methods_qDir} one can determine $\omega_0$ -- the direction of the corresponding lattice planes --
% \footnote{
%     Note that the \enquote{$-$} in $-q_\parallel/q_\perp$ is necessary, such that this fraction is the tangens of the angle between $\mathbf{q}$ and surface normal with correct sense of rotation. 
% },
...
Therefore, the mosaicity can be approximately quantified by the broadening of a reflection perpendicular to the direction of $\mathbf{K}_{hkl}$.
...
\begin{enumerate}
    ...
    \item If the thin film grows tilted (e.g.\ due to relaxed growth, cf.\ section \ref{Sec:Theory_Relaxed}), a symmetric reflection -- having scattering vector parallel to the surface normal in theory -- will exhibit an in-plane component $q_{\parallel,hkl}^\mathrm{film}\neq0$.
    ...
    \end{equation}
    To determine the lattice constants from asymmetric peaks, the reciprocal space is again rotated as in \eqref{Equ:Methods_rotation} but with rotation angle $-\theta_\mathrm{tilt}$.
...
For the results presented in this work, the radiation was produced by an copper anode, resulting in a wavelength of $\lambda=\qty{1.5406}{\angstrom}$ for \ce{Cu}-K\textalpha\textsubscript{1} radiation.
Note that \ce{Cu}-K\textalpha\textsubscript{2} and \ce{Cu}-K\textbeta\ radiation are not filtered out, resulting in additional low-intensity peaks in the diffractograms.
...
For 2\texttheta-\textomega-scans, the detecor is operating in \texttt{Scanning Line} mode.
For scans fixing the 2\texttheta\ position, i.e.\ \textomega- and \textphi-scans, the detector is operated in \texttt{Receiving Slit} mode.
...
For example, prior to a 2\texttheta-\textomega-scan of \textit{m}-plane oriented rhombohedral samples, a \textphi-scan has been applied for the inclined (30.6) reflection to find the correct azimuth of the \textit{c}-axis, which is called \textphi-optimization.
Afterwards, a Rocking curve has been recorded and $\omega$ set to the maximum of the peak to compensate for the expected lattice tilt, called an \textomega-optimization.
